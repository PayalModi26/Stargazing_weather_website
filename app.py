...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "PayalModi@26")
CORS(app)
...
def calculate_stargazing_score(cloud_cover, aqi, transparency, moon_illumination):
    """
    Weighted score:
      Cloud Cover   35%  (0% clouds → 100 pts)
      AQI           30%  (AQI ≤ 50 → 100 pts, linear decay)
...
def best_viewing_window(sunset_hour, cloud_cover, aqi):
    """Simple heuristic: start 1.5 h after sunset, 2-3 h window."""
...
def fetch_weather_aqi(city):
    ...
    a = requests.get(a_url, timeout=5).json()

    # 5-day forecast
    f_url = f"https://api.openweathermap.org/data/2.5/forecast?lat={lat}&lon={lon}&appid={OWM_API_KEY}&units=metric&cnt=40"
    f = requests.get(f_url, timeout=5).json()

    aqi_val = a["list"][0]["main"]["aqi"] * 50  # OWM 1-5 → approx 0-250
    components = a["list"][0]["components"]

    # Sunset hour (local approximation)
    ...
    weather_data = {
        ...
        "wind_speed":   round(w["wind"]["speed"] * 3.6, 1),  # m/s → km/h
...
def aqi_level(aqi):
    if aqi <= 50:   return {"label": "Good",            "color": "#00e400", "advice": "Air quality is excellent. Great for outdoor activities!"}
    if aqi <= 100:  return {"label": "Moderate",        "color": "#ffff00", "advice": "Acceptable. Unusually sensitive people should limit prolonged exertion."}
    if aqi <= 150:  return {"label": "Unhealthy for Sensitive Groups", "color": "#ff7e00", "advice": "Sensitive groups may experience effects. General public unaffected."}
    if aqi <= 200:  return {"label": "Unhealthy",       "color": "#ff0000", "advice": "Everyone may begin to experience effects. Limit prolonged outdoor exertion."}
    if aqi <= 300:  return {"label": "Very Unhealthy",  "color": "#8f3f97", "advice": "Health alert! Everyone may experience more serious effects."}
...
def dashboard():
    ...
    # Use OWM cloud cover as primary
